 ...
 * output tensor.  AIV uses GetSubBlockId to determine its lane (0=left,
...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t* args) {
    ...
    int32_t sub_block_id = GetSubBlockId(args);
    // AIV writes at cache line (1 + sub_block_id), skipping AIC's cache line 0
    int32_t offset = (1 + sub_block_id) * FLOATS_PER_CACHE_LINE;

    out[offset + 0] = static_cast<float>(GetBlockIdx(args));
    out[offset + 1] = static_cast<float>(GetBlockNum(args));

...
struct GlobalContext {
    int32_t sub_block_id; /**< AIV lane within cluster: 0=AIV0(left), 1=AIV1(right).
 ...
 */
static __aicore__ inline int32_t GetSubBlockId(__gm__ int64_t* args) {
 ...
 * Range: [0, GetBlockNum(args)).
 * Within the same task, different blocks receive different indices.
 */
static __aicore__ inline int32_t GetBlockIdx(__gm__ int64_t* args) {
 ...
 */
static __aicore__ inline int32_t GetBlockNum(__gm__ int64_t* args) {
    __gm__ LocalContext* ctx =
        reinterpret_cast<__gm__ LocalContext*>(static_cast<uint64_t>(args[SPMD_LOCAL_CONTEXT_INDEX]));
